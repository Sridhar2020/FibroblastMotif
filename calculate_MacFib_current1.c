...
double calculate_MacFib_current1( double *U1, double dt)
{
/* Indices for u array */
  int fib_volt = 21;
  int rkv = 22;
...
/* Concentrations */
 ...
 double FIB_Nao = 130.0;
   ...
   /* Fibroblast sodium reversal potentials */
    ...
    fib_alpha_K1 = 0.1/(1+exp(0.06*(U1[fib_volt]-fib_Ek-200)));
    fib_beta_K1= 3*exp(0.0002*(U1[fib_volt]-fib_Ek+100))+exp(0.1*(U1[fib_volt]-fib_Ek-10))/(1+exp(-0.5*(U1[fib_volt]-fib_Ek)));
    fib_rec_IK1= fib_alpha_K1/(fib_alpha_K1 + fib_beta_K1);
    fib_IK1 = FIB_GK1*fib_rec_IK1*(U1[fib_volt]-fib_Ek);

    /*Na-K pump in the fibroblast*/
     fib_INaK = FIB_I_NAK*(FIB_Ko/(FIB_Ko+FIB_K_mK))*(pow(FIB_Nai,1.5)/(pow(FIB_Nai,1.5)+pow(FIB_K_mna,1.5)))*((U1[fib_volt]-V_rev)/(U1[fib_volt] - fib_B));  

   /*inward Kv current*/
     fib_r_bar = 1/(1 + exp(-(U1[fib_volt] + 20.0 -30)/11));
     fib_s_bar = 1/(1 + exp((U1[fib_volt] + 23.0 -30 )/7));
     tr1 = (U1[fib_volt] + 20 -30)/25.9;
     tr2 = (U1[fib_volt] + 23 -30)/22.7;  
     fib_tau_r = 20.3 + 138*exp(-(tr1*tr1));
     fib_tau_s = 1574 + 5268*exp(-(tr2*tr2));	    
     U1[rkv] = fib_r_bar - (fib_r_bar-U1[rkv])*exp(-dt/fib_tau_r);
     U1[skv] = fib_s_bar - (fib_s_bar-U1[skv])*exp(-dt/fib_tau_s); 
     fib_IKv = FIB_GKV*U1[rkv]*U1[skv]*(U1[fib_volt]-fib_Ek);       
   
  /*Background sodium current*/
     fib_IbNa = FIB_G_BNA*(U1[fib_volt] - Ena_f);    
