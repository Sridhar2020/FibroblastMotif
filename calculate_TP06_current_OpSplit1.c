...
double calculate_TP06_current_OpSplit1( double *U1, double dt, double **lookup, int celltype, double stimCurrent )
{
...
//  Parameters for IKATP
  ...
  /* Reversal potentials */
  	double Ena = RTonF*log(Nao/U1[Nai]);
	double Ek = RTonF*(log((Ko/U1[Ki])));
	double Eks = RTonF*(log((Ko+pKNa*Nao)/(U1[Ki]+pKNa*U1[Nai])));
	double Eca = 0.5*RTonF*(log((Cao/U1[Cai])));
...
/* lookup table indices */
	...
	// for INa shift by up to -3.4 mV
  	Vmhi = ceil(U1[V]+INa_Vshift) * gain + offset;
  	Vmlo_diff = U1[V]+INa_Vshift - floor(U1[V]+INa_Vshift);
  	Vmlo = floor(U1[V]+INa_Vshift) * gain + offset;
  ...
  /* lookup table code */
       ...
       */	
  	alpha_m = 1.0/(1.0+exp((-60.0-U1[V])/5.0));
	beta_m = 0.1/(1.0+exp((U1[V]+35.0)/5.0))+0.10/(1.0+exp((U1[V]-50.0)/200.0));
	tau_m = alpha_m * beta_m;
	m_inf = 1.0/((1.0+exp((-56.86-U1[V])/9.03))*(1.0+exp((-56.86-U1[V])/9.03)));

	if (U1[V] >= -40.0)
	    {
	    alpha_h = 0.0;
	    beta_h = (0.77/(0.13*(1.0 + exp(-(U1[V] + 10.66)/11.1))));
	    }
	else
	    {
	    alpha_h = (0.057 * exp(-(U1[V] + 80.0)/6.8));
	    beta_h = (2.7 * exp(0.079 * U1[V])+(3.1e5) * exp(0.3485 * U1[V]));
	    }
	tau_h = 1.0/(alpha_h + beta_h);
	h_inf = 1.0/((1.0 + exp((U1[V] + 71.55)/7.43))*(1.0 + exp((U1[V] + 71.55)/7.43)));

	if(U1[V] >= -40.0)
	    {
	    alpha_j = 0.;
	    beta_j = (0.6 * exp((0.057) * U1[V])/(1.0 + exp(-0.1 * (U1[V] + 32.0))));
	    }
	else
	    {
		alpha_j = (((-2.5428e4)*exp(0.2444*U1[V])-(6.948e-6)*exp(-0.04391*U1[V]))*(U1[V]+37.78)/(1.0+exp(0.311*(U1[V]+79.23))));
		beta_j = (0.02424*exp(-0.01052*U1[V])/(1.0+exp(-0.1378*(U1[V]+40.14))));
	    }
	tau_j = 1.0/(alpha_j + beta_j);
	j_inf = h_inf;
       	 
 
  	U1[M] = m_inf - ( m_inf - U1[M] ) * exp( -dt / tau_m );
  	U1[H] = h_inf - ( h_inf - U1[H] ) * exp( -dt / tau_h );
	U1[J] = j_inf - ( j_inf - U1[J] ) * exp( -dt / tau_j );

  	INa = GNa*U1[M]*U1[M]*U1[M]*U1[H]*U1[J]*(U1[V]-Ena);

  	Vmhi = ceil(U1[V]) * gain + offset;
  	Vmlo_diff = U1[V] - floor(U1[V]);
  	Vmlo = floor(U1[V]) * gain + offset;
  ...
  /* lookup table code */
	...
	if (U1[V] >= 0) tau_f *= tau_f_multiplier; // Vm >= 0 added 20/12/2010
    ...
    d_inf = 1.0/(1.0+exp((-8.0-U1[V])/7.5));
    ad = 1.4/(1.0+exp((-35.0-U1[V])/13.0))+0.25;
    bd=1.4/(1.0+exp((U1[V]+5.0)/5.0));
	cd=1.0/(1.0+exp((50.0-U1[V])/20.0));
    tau_d = ad * bd * cd;

    f_inf = 1.0/(1.0+exp((U1[V]+20.0)/7.0));
    af=1102.5*exp(-(U1[V]+27.0)*(U1[V]+27.0)/225.0);
	bf=200.0/(1.0+exp((13.0-U1[V])/10.0));
	cf=(180.0/(1.0+exp((U1[V]+30.0)/10.0)))+20.0;
   if(U1[V] < 0)
   {
    tau_f = af + bf + cf;
   }
  else if(U1[V] >= 0 )
   {
    ...
    f2_inf = 0.67/(1.0+exp((U1[V]+35.0)/7.0))+0.33;
    af2=562.0*exp(-(U1[V]+27.0)*(U1[V]+27.0)/240.0);
	bf2=31.0/(1.0+exp((25.0-U1[V])/10.0));
	cf2=80.0/(1.0+exp((U1[V]+30.0)/10.0));
	tau_f2 = af2 + af2 + af2;
  

	fCass_inf = 0.6/(1.0+(U1[CaSS]/0.05)*(U1[CaSS]/0.05))+0.4;
	tau_fCass = 80.0/(1.0+(U1[CaSS]/0.05)*(U1[CaSS]/0.05))+2.0;

     U1[D] = d_inf - (d_inf - U1[D]) * exp( -dt / tau_d );
  	U1[F] = f_inf - (f_inf - U1[F]) * exp( -dt / tau_f );
  	U1[F2] = f2_inf - (f2_inf - U1[F2]) * exp( -dt / tau_f2 );
  	U1[FCass] = fCass_inf - (fCass_inf - U1[FCass]) * exp( -dt / tau_fCass );

  	ICaL = GCaL_pH*GCaL_atp*GCaL*U1[D]*U1[F]*U1[F2]*U1[FCass]*4.0*(U1[V]-15.0)*(Frdy/RTonF)*(0.25*exp(2.0*(U1[V]-15.0)/RTonF)*U1[CaSS]-Cao)/(exp(2.0*(U1[V]-15.0)/RTonF)-1.0);
  ...
  /* lookup table code */
    ...
    xr1_inf = 1.0/(1.0+exp((-26.0-U1[V])/7.0));
    axr1 = 450.0/(1.0+exp((-45.0-U1[V])/10.0));
    bxr1 = 6.0/(1.0+exp((U1[V]+30.0)/11.5));
    tau_xr1 = axr1 * bxr1;

    xr2_inf = 1.0/(1.0+exp((U1[V]+88.0)/24.0));
    axr2 = 3.0/(1.0+exp((-60.0-U1[V])/20.0));
    bxr2 = 1.12/(1.0+exp((U1[V]-60.0)/20.0));
    tau_xr2 = axr2 * bxr2;
     
 
 	Gkr = GkrPar1; //GkrPar4;
 	U1[Xr1] = xr1_inf - (xr1_inf - U1[Xr1]) * exp( -dt / tau_xr1 );
 	U1[Xr2] = xr2_inf - (xr2_inf - U1[Xr2]) * exp( -dt / tau_xr2 );
	IKr = Gkr*sqrt(Ko/5.4)*U1[Xr1]*U1[Xr2]*(U1[V]-Ek);
  ...
  /* lookup table code */
	...
	tau_xs = lookup[k_xs_exp][Vmlo] + Vmlo_diff * (lookup[k_xs_exp][Vmhi] - lookup[k_xs_exp][Vmlo]);
 	...
 	xs_inf = 1.0/(1.0+exp((-5.0-U1[V])/14.0));
	axs = (1400.0/(sqrt(1.0+exp((5.0-U1[V])/6.0))));
	bxs = (1.0/(1.0+exp((U1[V]-35.0)/15.0)));
	tau_xs = axs * bxs + 80.0;
     	

	U1[Xs] = xs_inf - (xs_inf - U1[Xs]) * exp( -dt / tau_xs );

	//if (celltype == 0) { Gks = GksEndo; }
	//else if (celltype == 1) { Gks = GksMcell; }
	//else  {Gks = GksEpi; }
	Gks = GksPar1; //GksPar4;
	IKs = Gks*U1[Xs]*U1[Xs]*(U1[V]-Eks);

  /* Time independent K current */
  	Ak1 = 0.1/(1.0+exp(0.06*(U1[V]-Ek-200.0)));
	Bk1 = (3.0*exp(0.0002*(U1[V]-Ek+100.0))+exp(0.1*(U1[V]-Ek-10.0)))/(1.0+exp(-0.5*(U1[V]-Ek)));
	rec_iK1 = Ak1/(Ak1+Bk1);
	IK1 = GK1*rec_iK1*(U1[V] - Ek);

  /* Plateau K current */
  	GpK=GpKPar1; //GpKPar4;
	rec_ipK = 1.0/(1.0+exp((25.0-U1[V])/5.98));
	IpK=GpK*rec_ipK*(U1[V]-Ek);
  ...
  /* EPI only code */
	  ...
	  */    	
               ...
               r_inf = 1.0/(1.0+exp((20.0-U1[V])/6.0));
		s_inf = 1.0/(1.0+exp((U1[V]+20.0)/5.0));
		tau_r = 9.5*exp(-(U1[V]+40.0)*(U1[V]+40.0)/1800.0)+0.8;
		tau_s = 85.0*exp(-(U1[V]+45.0)*(U1[V]+45.0)/320.0)+5.0/(1.0+exp((U1[V]-20.0)/5.0))+3.0;
	       	
		Gto = GtoEpi;//GtoEpi;

	U1[S] = s_inf - (s_inf - U1[S]) * exp(-dt / tau_s);
	U1[R] = r_inf - (r_inf - U1[R]) * exp(-dt / tau_r);
	Ito = Gto*U1[R]*U1[S]*(U1[V]-Ek);
  ...
  /* ATP dependent K current */
	...
	IKatp = gkbaratp*(U1[V]-ekatp);
        IKatp = 0.0; 

  /* Na Ca exchanger */
	naca1 = knaca*(1.0/(KmNai*KmNai*KmNai+Nao3))*(1.0/(KmCa+Cao));
	naca2 = (1.0/(1.0+ksat*exp((nn-1.0)*VmoRTonF)));
	naca3 = (exp(nn*VmoRTonF)*U1[Nai]*U1[Nai]*U1[Nai]*Cao-exp((nn-1.0)*VmoRTonF)*Nao3*U1[Cai]*2.5);
	INaCa = naca_pH * naca1 * naca2 * naca3;

	//INaCa=knaca*(1.0/(KmNai*KmNai*KmNai+Nao3))*(1.0/(KmCa+Cao))*(1.0/(1.0+ksat*exp((nn-1.0)*VmoRTonF)))*(exp(nn*VmoRTonF)*U1[Nai]*U1[Nai]*U1[Nai]*Cao-exp((nn-1.0)*VmoRTonF)*Nao3*Cai*2.5);
  ...
  /* iNaK */
	...
	INaK=knak*(Ko/(Ko+KmK))*(U1[Nai]/(U1[Nai]+KmNa))*rec_iNaK;

  /* Plateau Ca current */
 	GpCa=GpCaPar1; //GpCaPar4;
  	IpCa=GpCa*U1[Cai]/(KpCa+U1[Cai]);
  ...
  /* intracellular ion concentrations */
	...
	kCaSR = maxsr-((maxsr-minsr)/(1.0+(EC/U1[CaSR])*(EC/U1[CaSR])));
	k1 = k1bar/kCaSR;
	k2 = k2bar*kCaSR;
	dRR = k4 * (1.0-U1[RR]) - k2*U1[CaSS]*U1[RR];
	U1[RR] += dt*dRR;
	U1[OO] = k1*U1[CaSS]*U1[CaSS]*U1[RR]/(k3+k1*U1[CaSS]*U1[CaSS]);
	Irel = Vrel*U1[OO]*(U1[CaSR]-U1[CaSS]);
	Ileak = Vleak*(U1[CaSR]-U1[Cai]);
	Iup = Vmaxup/(1.0+((Kup*Kup)/(U1[Cai]*U1[Cai])));
	Ixfer = Vxfer*(U1[CaSS] - U1[Cai]);

	CaCSQN = Bufsr*U1[CaSR]/(U1[CaSR]+Kbufsr);
	dCaSR = dt*(Iup-Irel-Ileak);
	bjsr = Bufsr-CaCSQN-dCaSR-U1[CaSR]+Kbufsr;
	cjsr = Kbufsr*(CaCSQN+dCaSR+U1[CaSR]);
	U1[CaSR] = (sqrt(bjsr*bjsr+4.0*cjsr)-bjsr)/2.0;

	CaSSBuf=Bufss*U1[CaSS]/(U1[CaSS]+Kbufss);
	dCaSS = dt*(-Ixfer*(Vc/Vss)+Irel*(Vsr/Vss)+(-ICaL*inversevssF2*CAPACITANCE));
	bcss = Bufss-CaSSBuf-dCaSS-U1[CaSS]+Kbufss;
	ccss = Kbufss*(CaSSBuf+dCaSS+U1[CaSS]);
	U1[CaSS] = (sqrt(bcss*bcss+4.0*ccss)-bcss)/2.0;

	CaBuf = Bufc*U1[Cai]/(U1[Cai]+Kbufc);
	dCai = dt*((-(IbCa+IpCa-2.0*INaCa)*inverseVcF2*CAPACITANCE)-(Iup-Ileak)*(Vsr/Vc)+Ixfer);
	bc = Bufc-CaBuf-dCai-U1[Cai]+Kbufc;
	cc = Kbufc*(CaBuf+dCai+U1[Cai]);
	U1[Cai] = (sqrt(bc*bc+4.0*cc)-bc)/2.0;
